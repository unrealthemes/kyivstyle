...
<label class="custom-radio-btn" for="shipping_method_%1$d_%2$s"></label>
								<label for="shipping_method_%1$d_%2$s">
								<p class="delivery-type">%5$s</p>
								</label>',
                    ...
                    <?php if ($method->id != "nova_poshta_shipping_method" && $chosen_method != "nova_poshta_shipping_method") : ?>
                        <?php
                        $WC_Checkout = new WC_Checkout();
                        $fields = $WC_Checkout->get_checkout_fields('billing');

                        $class = !empty($WC_Checkout->get_value('billing_address_1') ) ? ' littleLetter ' : '' ?>
                        <li class="form-row <?php echo $class ?>">
